...
bool layerStatChart::deserveChart()
{
    ...
    if (!aRes)
    {
        std::cout << "la couche " << mLay->Nom() << " ne mérite pas de graphique pour ses statistiques " << std::endl;
    }
    return aRes;
}
...
std::unique_ptr<WContainerWidget> layerStatChart::getChart(bool forRenderingInPdf)
{
    ...
    { // je pensais que le rendu pdf était en conflit avec les layouts, mais ça ne semble pas être ça...
...
std::string nth_letter(int n)
{
    ...
    if (n >= 1 && n <= 26)
    {
        aRes = "abcdefghijklmnopqrstuvwxyz"[n - 1];
    }
    return aRes;
}
...
void batonnetApt::paintEvent(Wt::WPaintDevice *paintDevice)
{
    ...
    for (auto &kv : mLayStat->StatSimple())
    {
        ...
        {
            std::string codeApt = "";
            if (aptFull2aptAcro.find(kv.first) != aptFull2aptAcro.end())
            {
                codeApt = aptFull2aptAcro.at(kv.first);
            }
