                                                                                                       ...
                                                                                                       session(), mDico(dico), polygValid(0), mBDFile(aFileDB)
{
    ...
    bCancel->clicked().connect([=]
                               {doJavaScript("acr_src.clear();map.addInteraction(draw);"); polygValid=0; });
    WPushButton *bUndo = cont->addNew<WPushButton>(WString::tr("cancelPoint"));
    bUndo->setMinimumSize("100%", " ");
    bUndo->clicked().connect([=]
                             { doJavaScript("draw.removeLastPoint();"); });
...
{
    // std::cout << "valid draw \n" << geojson << std::endl;
    //  Use boost::filesystem::unique_path to produce a safe temporary file name
