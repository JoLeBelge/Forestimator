...
import '/locationIndicator/src/animated_location_controller.dart';
import '/locationIndicator/src/widgets/accuracy_indicator_wrapper.dart';
import '/locationIndicator/src/widgets/location_indicator_wrapper.dart';
import '/locationIndicator/src/widgets/orientation_indicator_wrapper.dart';
import '/locationIndicator/src/widgets/accuracy_indicator.dart';
...
class _AnimatedLocationLayerState extends State<AnimatedLocationLayer>
  ...
  AnimatedLocationControllerImpl get _controller => (widget.controller ??
  ...
  double get _scale => _location != null
      ? _calculateMetersPerPixel(_location!.latitude, _mapCamera.zoom)
      : 1;
  ...
  void _handleAbsoluteOrientationEvent(SensorEvent event) {
    ...
    if (_isVisible) {
      if (defaultTargetPlatform == TargetPlatform.iOS) {
        ...
        final g = event.data;
        final norm =
            sqrt(g[0] * g[0] + g[1] * g[1] + g[2] * g[2] + g[3] * g[3]);
      ...
      // check if difference threshold is reached
      if (_orientation == null ||
          (_orientation! - newOrientation).abs() >
              widget.orientationDifferenceThreshold) {
        setState(() {
  ...
  void _updateCamera() async {
    ...
    if (widget.cameraTrackingMode != CameraTrackingMode.none &&
        _controller.location != null) {
      final controller = MapController.of(context);
      if (widget.cameraTrackingMode ==
              CameraTrackingMode.locationAndOrientation &&
