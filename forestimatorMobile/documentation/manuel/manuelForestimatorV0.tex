...
{
	name={layer},
	description={couches cartographiques permettant à l'utilisateur d'encoder des points ou des polygones. La dénomination technique exacte, évitée dans ce document par souci de simplicité, est textit{couche vectorielle}}
}
...
\begin{itemize}
    \item \textbf{À gauche :} \includegraphics[width=0.05\textwidth]{icons/tree.png} (icône d'arbre) pour ouvrir le menu des outils de géolocalisation. La géolocalisation est effectuée soit au moyen du GPS de votre smartphone, soit au moyen de recherche d'un lieu via son appellation.
...
\begin{itemize}
    \item \textbf{À gauche :} \includegraphics[width=0.05\textwidth]{icons/settings.png} (icône de paramètres) pour accéder aux réglages globaux et aux informations relatives à l'application.
...
Dans les paramètres, vous pouvez modifier les permissions accordées à l'application et consulter diverses informations. Les différentes sections reprises dans les paramètres sont les suivantes :
\begin{itemize}
...
Appuyez sur l'icône \includegraphics[width=0.05\textwidth]{icons/eye.png} de la barre de menu inférieur pour ouvrir le sélecteur de couche. Ce même bouton permet de cacher la fenêtre du sélecteur à l'issue du choix de la carte à afficher. Le sélecteur de couches est organisé en trois parties. La première en haut permet de contrôler la visibilité des couches de points et de polygones. La deuxième permet l'affichage de un à trois couche(s) thématique(s), et la troisième partie en bas permet un accès direct aux catalogues.
...
\begin{itemize}
	...
	\vspace{0.5cm}
	\item \textbf{Conditions stationnelles} : les couches de descriptions des \glspl{station} en liens avec le \gls{FEE}. Vous y trouverez les informations concernant le type de sol, le niveau hydrique et trophique ou encore la situation topographique des stations forestières. 
	\vspace{0.5cm}
...
En regardant de près l'encart d'une couche dans le catalogue, vous pouvez distinguer plusieurs petits symboles à gauche du nom de celle-ci. 
...
Les symboles localisés dans la partie gauche résument plusieurs caractéristiques qui sont plus amplement détaillées lorsque vous appuyez sur l'encart. Les caractéristiques d'une couche sont structurées de la manière suivante :
\begin{itemize}
...
\begin{instructionbox}{Modifier les options d'une couche}
	\begin{enumerate}
		\item En appuyant sur l'encart d'une couche, vous avez accès à sa description complète et vous pouvez modifier certaines propriétés : sélectionner la couche pour les analyses ponctuelles et surfacique, ou encore effectuer son téléchargement pour un usage en \gls{offline}.
	\end{enumerate}
\end{instructionbox}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Position des outils de géolocalisation (gauche) et icônes du menu de géolocalisation (droite) avec de de haut en bas : 1) analyse pour la position GPS actuelle du smartphone, 2) centrage de la carte sur la position GPS et 3) outil de recherche d'un lieu sur base de son nom.}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Résultat de recherche d'un lieu sur base de son nom, exemple avec Gembloux. Des marqueurs de position sont ajouté à la carte pour les lieux recherchés.}
\end{figure}
...
\subsection{Procédure de création d'une layer de points et d'ajout d'entités}
...
\begin{instructionbox}{Créer une nouvelle layer de points}
	\begin{enumerate}
		...
		\item Votre layer apparaît dans la liste la liste des couches cartographiques de points/polygones. 
	\end{enumerate}
\end{instructionbox}
...
\begin{instructionbox}{Ajouter des points}
	\begin{enumerate}
		...
		\item Déplacez la carte pour positionner le centre sur votre position d'intérêt et cliquez sur \includegraphics[width=0.05\textwidth]{icons/add_entity.png} en bas à droite de la carte pour ajouter un point. 
...
\subsection{Procédure de création d'une layer de polygones et d'ajout d'entités}
...
\begin{enumerate}
    ...
    \item Si vous appuyez à nouveau sur cette fenêtre d'aperçu, vous réaliserez une analyse avec la série de couches sélectionnées à cet effet.
\end{enumerate}
\end{instructionbox}
...
\begin{enumerate}
    ...
    \includegraphics[width=0.35\textwidth]{LayerList.png}
    \item Sélectionnez la layer de polygone et accédez au propriété de la layer en cliquant sur le bouton \includegraphics[width=0.05\textwidth]{icons/arrow.png}.
...
\subsection{Procédure de sélectionner une couche thématique pour l'analyse}

Les analyses ponctuelles et surfaciques sont des analyses effectuées sur une série de \glspl{couche}. Cette série est prédéfinie, mais vous pouvez ajouter d'autre couche de votre intérêt de manière à les utiliser lors de vos analyses.
