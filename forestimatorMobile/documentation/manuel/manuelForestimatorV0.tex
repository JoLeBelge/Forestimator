...
\subsection{Procédure : Sélectionner une couche thématique depuis un des trois encarts du sélecteur}
\begin{instructionbox}{Sélectionner une couche thématique}
\begin{enumerate}
    ...
    \item Le catalogue s'affiche et vous permet de naviger parmi les différentes couches thématiques. Si vous appuyez sur l'icône \includegraphics[width=0.05\textwidth]{icons/layers.png}, vous pouvez sélectionner cette couche spécifique. Un cercle vert avec un numéro de 1 à 3 apparaît alors à la place de l'icône. Ce numéro représente l'ordre d'affichage de la couche.
\end{enumerate}
\end{instructionbox}
...
\begin{enumerate}
    ...
    \item Appuyez sur le bouton \textit{Fermer} pour fermer le catalogue.
\end{enumerate}
\end{instructionbox}
...
\begin{enumerate}
    ...
    \item Depuis l'écran principal de Forestimator, appuyez simplement quelque part sur la carte.
    \item Un aperçu court en une ligne apparaît et vous indique pour cette position la légende et la couleur de la couche cartographique numéro 1.
    \item Si vous appuyez à nouveau sur cette fenêtre d'aperçu, vous réaliserez une analyse avec une série de couches sélectionnées. En mode en ligne, l'analyse sera effectuée sur nos serveurs et en mode hors ligne, elle sera calculée sur votre smartphone avec les couches que vous avez téléchargées préalablement.
    \item Alternativement, vous pouvez effectuer un appui long sur la carte pour obtenir directement une analyse complète de toutes les couches sélectionnées.
\end{enumerate}
\end{instructionbox}

\begin{instructionbox}{Analyse ponctuelle en une étape}
	\begin{enumerate}
		\item Depuis l'écran principal de Forestimator, effectuer un appui long sur la carte pour obtenir directement une analyse complète de toutes les couches sélectionnées.
	\end{enumerate}
\end{instructionbox}
...
%-exo : mesurer la hauteur d'un arbre avec le MNH et puis avec la couche dendro et l'analyse ponctuelle.
