    ...
    session()
{
   ...
   // std::string q="SELECT COUNT(*)as nb, cat FROM log  WHERE ip != '127.0.0.1' AND ip NOT LIKE '%139.165%' GROUP BY cat;";
    ...
    for (int m(1);m <13;m++){
        ...
        table4->elementAt(m,0)->addWidget(std::make_unique<Wt::WText>(month));
        table4->elementAt(m,0)->setContentAlignment(AlignmentFlag::Right);
        table4->elementAt(m,1)->addWidget(std::make_unique<Wt::WText>(std::to_string(nb)));
    ...
    for (int m(1);m <6;m++){
        ...
        table4->elementAt(m+12,0)->addWidget(std::make_unique<Wt::WText>(month));
        table4->elementAt(m+12,0)->setContentAlignment(AlignmentFlag::Right);
        table4->elementAt(m+12,1)->addWidget(std::make_unique<Wt::WText>(std::to_string(nb)));
