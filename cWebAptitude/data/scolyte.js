...
var extent = [42247, 21148, 295176, 167719];// full RW
...
/*var o2016 = new ol.layer.Tile({
	title: 'Ortho 2016',
	extent: extent,
	source: new ol.source.TileWMS({
		title: 'Ortho 2016',
		url: 'http://gxgfservcarto.gxabt.ulg.ac.be/cgi-bin/map',
		crossOrigin: 'null',
		attributions: 'Â© CartoWeb.be & Geoportail.wallonie.be',
		params: {
			'LAYERS': 'RGB_2016 0.25',
			'FORMAT': 'image/jpeg'},
...
var globale_ndvi_19_06 = new ol.layer.Tile({
	extent: extend_previous,
	title: 'Indice de vÃ©gÃ©tation global',
	source: new ol.source.TileWMS({
		url: 'https://gxgfservcarto.gxabt.ulg.ac.be/cgi-bin/scolyte_rw',
		crossOrigin: 'null',
		attributions: 'Â© Gembloux Agro-Bio Tech - TERRA',
		params: {
		  'LAYERS': 'NDVI_19_06',
		  'TILED': true,
		  'FORMAT': 'image/jpeg'
...
var planet_ir_19_06 = new ol.layer.Tile({
	extent: extend_previous,
	title: 'Composition colorÃ©e',
	source: new ol.source.TileWMS({
		title: 'Composition colorÃ©e',
		url: 'https://gxgfservcarto.gxabt.ulg.ac.be/cgi-bin/scolyte_rw',
		crossOrigin: 'null',
...
var globale_ndvi_19_10 = new ol.layer.Tile({
	extent: extend_previous,
	title: 'Indice de vÃ©gÃ©tation global',
	source: new ol.source.TileWMS({
		url: 'https://gxgfservcarto.gxabt.ulg.ac.be/cgi-bin/scolyte_rw',
...
var planet_ir_19_10 = new ol.layer.Tile({
	extent: extend_previous,
	title: 'Composition colorÃ©e',
	source: new ol.source.TileWMS({
		title: 'Composition colorÃ©e',
...
var ndvi_19_06 = new ol.layer.Tile({
	preload: 1,
	title: 'Indice de vÃ©gÃ©tation pour les Ã©picÃ©as',
	source: new ol.source.TileImage({
		crossOrigin: null,
		extent: extend_ndvi,
		tileGrid: new ol.tilegrid.TileGrid({
			extent: extend_ndvi,
			origin: [extend_ndvi[0], extend_ndvi[1]],
			resolutions: res_ndvi,
			tileSize: [397, 397]
		}),
		tileUrlFunction: function (coordinate) {
			...
			var url = 'retiled201906/' + z + '/' + y + '/NDVI_NoDataFilled_Masked_Picea(60)_MNH2018(800)_CC_L72_BBOXed_' + _y + '_' + _x + '.tif';
			console.log(url);
...
var ndvi_19_10 = new ol.layer.Tile({
	preload: 1,
	title: 'Indice de vÃ©gÃ©tation pour les Ã©picÃ©as',
	source: new ol.source.TileImage({
		crossOrigin: null,
		...
		tileUrlFunction: function (coordinate) {
			...
			//var url = 'retiled201910/' + z + '/' + y + '/NDVI_NoDataFilled_Masked_Picea(60)_MNH2018(800)_CC_L72_BBOXed_' + _y + '_' + _x + '.tif';
...
var scolyte_pt = new ol.layer.Vector({
	crossOrigin: 'null',
	title: 'Foyer d\'arbres dÃ©pÃ©rissants',
	extent: extent,
	source: new ol.source.Vector({
		format: new ol.format.GeoJSON(),
		url: function(extent) {
		  return 'https://gxgfservcarto.gxabt.ulg.ac.be/cgi-bin/scolyte_shp?' +
			  'SERVICE=WFS&version=2.0.0&request=GetFeature&typename=scolytes&' +
			  'outputFormat=geojson&srsname=EPSG:31370&' +
			  'bbox=' + extent.join(',') + ',EPSG:31370';
		},
		strategy: ol.loadingstrategy.bbox
...
/*  Style couleurs differentes pour les scolytes
new Style({
...
var styleCache = {};
var styleColors = ["#ffffb2","#fecc5c","orange","#fd693c","#f03b20","#bd0026"]
var styleFunction = function(feature) {
	var c = feature.get('c');
	var style = styleCache[c];
	if (!style) {
		console.log(c+" "+styleColors[c]);
		style = new Style({
...
var layers = [
	...
	new ol.layer.Group({
		'title': 'Situation septembre 2019',
		layers:[ planet_ir_19_10, globale_ndvi_19_10, ndvi_19_10 ]
	}),
	new ol.layer.Group({
		'title': 'Situation juin 2019',
		layers:[ planet_ir_19_06, globale_ndvi_19_06, ndvi_19_06 ]
	}),
	new ol.layer.Group({
		'title': 'Situation novembre 2018',
		layers:[ scolyte_pt ]
	}),
	new ol.layer.Group({
...
planet_ir_19_10.setVisible(false);
planet_ir_19_06.setVisible(false);
IGN.setVisible(false);
scolyte_pt.setVisible(false);
parcelles.setVisible(false);
globale_ndvi_19_10.setVisible(false);
globale_ndvi_19_06.setVisible(false);
ndvi_19_06.setVisible(false);

// pas implémenté dans ol3! bien ben c'est ce que je dois implémenter moi-meme dans wt
...
/*map.on('singleclick', function(evt) {
	...
	content.innerText = "Frequence : "+properties["Freq"];
	console.log("Frequence : "+properties["Freq"]);
...
function centrer_com(){
	...
	//var poly = new Polygon([[213307,134406],[219526,134406],[219526,142795],[213307,142795],[213307,134406]]);
	//alert(poly.getArea());
	//return;
	//var feature = commune_src.getFeatures()[0];
	//alert(feature.getGeometry());
	//_view.centerOn(pt, map.getSize(), [800,400]);
	_view.fit(_tab_com[selek.selectedIndex], {padding: [10, 10, 10, 10],constrainResolution: false});
	//_view.fit([213307,134406,219526,142795],{size: map.getSize()});
	//_view.fit(feature.getGeometry(),{size: map.getSize()});
	//_view.fit(poly,{size: map.getSize()});
	//_view.fit([[213307,134406],[219526,134406],[219526,142795],[213307,142795],[213307,142795]]);
}
var bs = document.getElementById('selek_but');
bs.onclick = function(){centrer_com();};


/*	Assign new drawer for ndvi layers	*/
var olgt_ndvi_19_10 = new olGeoTiff(ndvi_19_10);
olgt_ndvi_19_10.plotOptions.domain = [1, 10];
olgt_ndvi_19_10.plotOptions.noDataValue = 0;
olgt_ndvi_19_10.plotOptions.palette = "hsv";//"inferno";greens

var _domain_min=0;
var _domain_max=30;

olgt_ndvi_19_10.plotOptions.dataFunction = function(b) {
	
	if(b[1]==0) return 0; // return 0 as nodata value
	if(b[1]<_domain_min) return 0; // return 0 as nodata value
	if(b[1]>_domain_max) return 0; // return 0 as nodata value
	
	return ( b[1] ); 
}
olgt_ndvi_19_10.redraw();

olgt_ndvi_19_06 = new olGeoTiff(ndvi_19_06);
olgt_ndvi_19_06.plotOptions.domain = [1400, 1900];
olgt_ndvi_19_06.plotOptions.noDataValue = 0;
olgt_ndvi_19_06.plotOptions.palette = "hsv";//"inferno";greens
olgt_ndvi_19_06.plotOptions.dataFunction = function(b) {
...
$(document).ready(function() {
	...
	// slider1 (domain)
	  ...
	  noUiSlider.create(slider, {
		start: [1625,1775],//olgt_ndvi.plotOptions.domain,
		connect: true,
		range: { 'min': 1400, 'max': 1900 },
		tooltips: true,
		format: wNumb({
			decimals: 0,
			thousand: '',
	  ...
	  slider.noUiSlider.on('change', function(values) {
		_domain_min=values[0];
		_domain_max=values[1];
		//olgt_ndvi.plotOptions.domain = [values[0], values[1]];
		olgt_ndvi_19_10.redraw();
		olgt_ndvi_19_06.redraw();
	  });
