...
siglePedo::siglePedo(std::string texture, std::string drainage,std::string profil, std::string charge, int profIPRFW, cDicoCartepH * dico):
  mMAT_TEXT(texture),mDRAINAGE(drainage),mDEV_PROFIL(profil),mPHASE_2(""),mPHASE_4(""),mPHASE_5(""),mPHASE_6(""), INDEX_(0),mSER_SPEC(""){
    if (charge=="*"){charge="";}
    mCHARGE=charge;
    std::vector<std::string> lut{"6","3","2","2","0_1"};
    mPHASE_1=lut.at(profIPRFW-1);
    mMAT_TEXT_SIMP=dico->getMatTextSimp(mMAT_TEXT);
    prepare();
}
